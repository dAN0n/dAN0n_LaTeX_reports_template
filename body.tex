...
Слишком потно, надо очень хорошо знать, что делаешь, иначе можно потратить день и не добиться результата. В документации 1200 страниц, Карл!!!\cite{pgf}
...
Использует пакет $mathtools$.\cite{mathtools}
...
Нельзя использовать зачёркивания пакета $cancel$\cite{cancel} с самого первого слова: \\
