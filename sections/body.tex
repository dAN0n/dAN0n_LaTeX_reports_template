...
Случай с подписями на разной высоте при использовании случая выше можно решить так (\vref{pic:results}).

\begin{figure}[H]
	\minipage{.3\textwidth}\centering \includegraphics[width=0.5\textwidth]{result}		\endminipage%
	\minipage{.7\textwidth}\centering \includegraphics[width=0.5\textwidth]{result2}	\endminipage%

	\begin{subfigure}[b]{.3\textwidth} \caption{} \label{pic:result}	\end{subfigure}%
	\begin{subfigure}[b]{.7\textwidth} \caption{} \label{pic:result2}	\end{subfigure}%
...
Пакет \emph{autonum} меняет работу с формулами. \cite{autonum} Например, он удаляет окружение \string\begin\{equation*\} и делает команду \string\[\string\] идентичной окружению \string\begin\{equation\}. При этом номера показываются только у тех формул, на которые присутствует ссылка, что немного упрощает работу. Следует обратить внимание на то, что \textbf{для верного отображения ссылки может потребоваться 3 компиляции вместо 2-х}.
...
\begin{enumerate}
	...
	Тогда используйте \string\cref\{\} и \string\Cref\{\}. Тип ссылки подставляется автоматически (для листинга с \string\cref\{\} нет).

	\item Вы ссылаетесь на формулу.

	Изначально были проблемы, но в процессе работы над шаблоном команды \string\vref\{\} и \string\Vref\{\} были переопределены, и теперь все нормально. Ссылка будет вида "`\vref{equ:rus}"'.

	\item Вы ссылаетесь на листинг.

	Изначально были проблемы, но в процессе работы над шаблоном команды \string\vref\{\} и \string\Vref\{\} были переопределены, и теперь все нормально. Тип ссылки для листинга с \string\vref\{\} не подставляется.
\end{enumerate}
